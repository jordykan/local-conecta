...
async function verifyOwnership(businessId: string) {
  ...
  if (!business) return { supabase, user, error: "No tienes permiso para este negocio." }

  return { supabase, user, business, error: null }
}

export async function createProduct(businessId: string, data: unknown) {
  const { supabase, error: authError, business } = await verifyOwnership(businessId)
  if (authError || !business) return { error: authError ?? "Error de autorizacion." }
...
export async function updateProduct(productId: string, data: unknown) {
  ...
  // Get product's business, verify ownership
